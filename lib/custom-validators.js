...
/**
 * defineRuleSchema - Checks that a schema being used for a rule is valid 
 *  
 * @param  {type} ajv         description 
 * @param  {type} ajvSchemaId description 
 * @return {type}             description 
 */ 
function defineRuleSchema(ajv, ajvSchemaId) {
  return function(dataToValidate) {
    const valid = ajv.validateSchema(dataToValidate);
    if (!valid) throw new ValidationError(ajvSchemaId, ajv.errors); 
  };
}
