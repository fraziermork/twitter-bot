...
module.exports = function(TwitterBot){
  ...
  TwitterBot.prototype._handleStreamingTweetEvent = function(tweet) {
    ...
    this.produceProcessedTweetText(tweet);//need to use call so that it can read the values of the options
    
    for (var i = 0; i < this._streamingApiRouterKeys.length; i++) {
      if (tweet.processed_tweet_text.match(this._streamingApiRouter[this._streamingApiRouterKeys[i]]['regexRouter'])){
        console.log('It matched with ' + this._streamingApiRouterKeys[i]);
        return this._streamingApiRouter[this._streamingApiRouterKeys[i]]['currentBot']['_handleTweet'](tweet);
      }
    }
  };
};
