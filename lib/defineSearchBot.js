...
module.exports = function(parent){
  ...
  parent.__proto__.defineSearchBot = function(content, stringToSearchFor, callback, optionsObj){ //this is the function that they'll use to
    ...
    try { //to define new bot
      if(content && stringToSearchFor){
        ...
        // console.log(parent.bots.searchBots[newSearchBot.botName].botName + ' attached to parent.');
        if(callback){
  ...
  function _SearchBot(content, stringToSearchFor, optionsObj){
    ...
    // this.parent                           = parent; //TODO: do we want to do this?
    this.content                          = content;
    this.stringToSearchFor                = stringToSearchFor;
    this._matchStrings                    = stringToSearchFor.replace(/\,\ +/, ',', 'g').split(',');  //removes all instances of a comma followed by a 
    this._regexRouterString               = null;
    this._running                         = false;
    ...
    //properties related to tweet processing for this bot 
    this.removeUrlsFromTweets             = true;
    this.removeMentionsFromTweets         = true;
    this.removeHashtagsFromTweets         = true;
    this.removeSymbolsFromTweets          = true;
    this.removeExtendedEntitiesFromTweets = true;
    this.removeMediaFromTweets            = true;
    ...
    //build the regex string that will be used for 
    this._regexRouterString = _.escapeRegExp(this._matchStrings).map((andStr) => {
      return andStr.split(' ').map((str) => {
        return '(?=.*' + str + ')';
      }).join('');
    }).join('|');
    
    console.log('this._regexRouterString is ', this._regexRouterString);
  ...
  _SearchBot.prototype._handleTweet = function(tweet){
    ...
    parent.produceProcessedTweetText.call(this, tweet);//need to use call so that it can read the values of the options
