...
module.exports = function(parent){
  ...
  _SearchBot.prototype._returnTweetString = function(tweet){
    console.log('_SearchBot.prototype._returnTweetString called');
  ...
  _SearchBot.prototype._handleTweet = function(tweet){
    console.log('_SearchBot.prototype._handleTweet called');
    if(!this.running){
      return;
    }
    try {
      if(this.content instanceof Function){
        let stringGenerationPromise = this._returnTweetString();
        stringGenerationPromise
        .then((stringToTweet) => {
          parent.tweet(stringToTweet, this.callback, this.tweetOptions);
        })
        .catch((err) => {
          throw new Error(err);
        });
      } else if (typeof this.content === 'string' || typeof this.content === 'number'){
        parent.tweet(this.content, this.callback, this.tweetOptions);
      } else {
