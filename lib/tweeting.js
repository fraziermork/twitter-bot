...
module.exports = function(TwitterBot){
  ...
  TwitterBot.prototype.produceProcessedTweetText = function(tweet){
    ...
    let textWithoutEntities = tweet.text; //TODO: enable option to not deburr, also look into how this interacts with non-english languages, maybe make it so that this isn't automatically used if not specifically looking for english?
    ...
    console.log('tweet.entities is');
    console.dir(tweet.entities);
    
    //iterate through each entity type and remove it from the tweet text
    Object.keys(removalInfo).forEach((currentEntityTypeName) => {
      if (removalInfo[currentEntityTypeName]['optionToCheck'] && tweet.entities[currentEntityTypeName]){
        ...
        tweet.entities[currentEntityTypeName].forEach((currentEntityObj) => {
          ...
          let stringToReplace = _.trim(stringToPrepend + currentEntityObj[propToRemove]);
          console.log('StringToReplace is ', stringToReplace);
          textWithoutEntities = textWithoutEntities.replace(stringToReplace, '', 'ig');
          console.log('textWithoutEntities after replace is ', textWithoutEntities);
          return;
        });
      } else {
        // console.log("removalInfo[currentEntityTypeName]['optionToCheck'] ", removalInfo[currentEntityTypeName]['optionToCheck']);
        // console.log("tweet.entities[currentEntityTypeName] ", tweet.entities[currentEntityTypeName]);
        return;
      }
    });
    
    //assign the fully replaced text as a new property on the tweet object
    tweet.processed_tweet_text = textWithoutEntities.replace('RT : ', '');
    console.log('final textWithoutEntities is ' + tweet.processed_tweet_text);
