...
TwitterBot.prototype._checkIfAReplyBotIsRunning = function(){
  ...
  for (var i = 0; i < this.bots.replyBots.length; i++) {
...
TwitterBot.prototype._updateStreamingApiRouter = function(){
  ...
  let activeReplyBotName = Object.keys(this.bots.replyBots).filter((currentReplyBotName) => {
    let currentReplyBot = this.bots.replyBots[currentReplyBotName];
    return currentReplyBot instanceof Object && currentReplyBot._running && !currentReplyBot.RESTOnly;
  })[0];
  self._streamingApiRouter['@' + this.screen_name] = this.bots.replyBots[activeReplyBotName];
