...
TwitterBot.prototype._checkIfAReplyBotIsRunning = function(){
  ...
  let replyBots = Object.keys(this.bots.replyBots);
  for (var i = 0; i < replyBots.length; i++) {
    if (replyBots[i]._running){
      return true;
    }
  }
  return false;
...
TwitterBot.prototype._collectAllSearchBotMatchStrings = function(){
  ...
  return _.flatten(_.compact(Object.keys(this.bots.searchBots).map((currentSearchBot) => { 
    if(currentSearchBot instanceof Object && currentSearchBot._running){
...
//This is used to update the streaming api router to make sure that the router only has properties and methods attached to it 
TwitterBot.prototype._updateStreamingApiRouter = function(){
  console.log('TwitterBot.prototype._updateStreamingApiRouter called');
  let self = this;
  this._streamingApiRouter = {};
  Object.keys(this.bots.searchBots).forEach((currentSearchBot) => {
    if(!(currentSearchBot instanceof Object)){
      return;
    }
...
TwitterBot.prototype._startApiStream = function(){
  ...
  this._updateStreamingSearchParameters();
  this._updateStreamingApiRouter();
  this._searchStream = this.twit.stream('statuses/filter', this._stringToTrack);
  this._searchStream.on('tweet', this._handleStreamingTweetEvent);
};
